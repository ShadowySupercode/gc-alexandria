...
<script lang="ts">
  ...
  let profileLoadingProgress = $state<{
    current: number;
    total: number;
  } | null>(null);
  ...
  async function fetchFollowLists(
    ...
    debug(
      `Fetching kind 3 follow lists with limit ${config.limit}, depth ${depth}`,
    );
    ...
    if (config.limit === 1) {
      ...
      if (followedPubkeys.length > 0) {
        ...
        debug(
          `Fetching ${pubkeysToFetch.length} additional follow lists (total limit: ${config.limit})`,
        );
    ...
    if (depth > 0) {
      ...
      for (let level = 1; level <= depth; level++) {
        ...
        debug(
          `Fetching level ${level} follow lists for ${currentLevelPubkeys.length} pubkeys`,
        );
  ...
  async function fetchEvents() {
    ...
    try {
      ...
      const { allConfigs, publicationConfigs, otherConfigs, kind0Config } =
        setupFetchConfiguration();

      // Step 2: Fetch non-publication events
      const nonPublicationEvents =
        await fetchNonPublicationEvents(otherConfigs);

      // Step 3: Fetch publication index events
      const validIndexEvents =
        await fetchPublicationIndexEvents(publicationConfigs);

      // Step 4: Extract and fetch content events
      const contentEvents = await fetchContentEvents(
        validIndexEvents,
        publicationConfigs,
      );
      ...
      const eventsWithProfiles = await fetchProfilesForEvents(
        combinedEvents,
        kind0Config,
      );
  ...
  function setupFetchConfiguration() {
    ...
    debug(
      "Enabled kinds:",
      allConfigs.filter((ec) => ec.enabled !== false).map((ec) => ec.kind),
    );
    ...
    const publicationConfigs = allConfigs.filter((ec) =>
      publicationKinds.includes(ec.kind),
    );
    const otherConfigs = allConfigs.filter(
      (ec) => !publicationKinds.includes(ec.kind),
    );
   ...
   */
  async function fetchNonPublicationEvents(
    otherConfigs: any[],
  ): Promise<NDKEvent[]> {
    ...
    if (nonProfileConfigs.length > 0) {
      debug(
        "Fetching non-publication events (excluding profiles):",
        nonProfileConfigs,
      );
      ...
      for (const config of nonProfileConfigs) {
        try {
          ...
          if (config.kind === 3) {
            ...
            debug(
              `Fetched ${fetchedEvents.size} events of kind ${config.kind}`,
            );
   ...
   * Step 3: Fetch publication index events
   */
  async function fetchPublicationIndexEvents(
    publicationConfigs: any[],
  ): Promise<Set<NDKEvent>> {
    const shouldFetchIndex = publicationConfigs.some(
      (ec) => ec.kind === INDEX_EVENT_KIND,
    );
    ...
    if (data.eventId) {
      ...
      if (!event) {
        ...
        throw new Error(
          `Publication not found: ${data.eventId}. The event may not be available on any of the configured relays.`,
        );
      }
      ...
      if (event.kind !== INDEX_EVENT_KIND) {
        throw new Error(
          `Event ${data.eventId} is not a publication index (kind ${INDEX_EVENT_KIND})`,
        );
      }
      ...
      const indexConfig = publicationConfigs.find(
        (ec) => ec.kind === INDEX_EVENT_KIND,
      );
   ...
   * Step 4: Extract and fetch content events
   */
  async function fetchContentEvents(
    ...
    const enabledContentKinds = CONTENT_EVENT_KINDS.filter((kind) =>
      enabledPublicationKinds.includes(kind),
    );
    debug(
      `Fetching content events (enabled kinds: ${enabledContentKinds.join(", ")})`,
    );

    // Group by author to make more efficient queries
    const referencesByAuthor = groupContentReferencesByAuthor(
      contentReferences,
      enabledContentKinds,
    );
  ...
  function extractContentReferences(
    validIndexEvents: Set<NDKEvent>,
  ): Map<string, { kind: number; pubkey: string; dTag: string }> {
  ...
  function groupContentReferencesByAuthor(
    ...
    const referencesByAuthor = new Map<
      string,
      Array<{ kind: number; dTag: string }>
    >();
   ...
   * Step 6: Fetch profiles for discovered pubkeys
   */
  async function fetchProfilesForEvents(
    combinedEvents: NDKEvent[],
    kind0Config: any,
  ): Promise<NDKEvent[]> {
    ...
    const shouldIncludeFollowPubkeys =
      followListConfig && followListConfig.limit > 0;
    ...
    if (shouldIncludeFollowPubkeys && followListEvents.length > 0) {
      ...
      debug(
        "Excluding follow list pubkeys (limit = 0, only fetching event authors)",
      );
    }
    ...
    if (kind0Config) {
      ...
      debug(
        "Profile fetch complete, fetched",
  ...
  function updateFinalState(
    ...
    debug("Events after expansion:", {
      ...
      searchMode: $visualizationConfig.searchThroughFetched
        ? "fetched"
        : "relays",
    });
  }
  ...
  async function handleTagExpansion(tags: string[]) {
    debug("Handling tag expansion", {
      tags,
      searchThroughFetched: $visualizationConfig.searchThroughFetched,
    });
    ...
    try {
      ...
      await fetchProfilesForNewEvents(
        ...
        (progress: { current: number; total: number } | null) => {
          profileLoadingProgress = progress;
        },
        debug,
      );
...
<div class="leather w-full p-4 relative">
  ...
  {#if loading}
    <div class="flex flex-col justify-center items-center h-64 gap-4">
      {debug(
        "TEMPLATE: Loading is true, events.length =",
        events.length,
        "allEvents.length =",
        allEvents.length,
      )}
          ...
          <div class="mt-4">
            ...
            <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
              <div
                ...
                style="width: {(profileLoadingProgress.current /
                  profileLoadingProgress.total) *
                  100}%"
              ></div>
            </div>
          </div>
