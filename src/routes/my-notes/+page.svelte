<script lang="ts">
  ...
  const tagTypes = ['t', 'title', 'm']; // 'm' is MIME type
  ...
  async function fetchMyNotes() {
    ...
    try {
      ...
      events = Array.from(eventSet).filter((e): e is NDKEvent => !!e && typeof e.created_at === 'number').sort((a, b) => (b.created_at ?? 0) - (a.created_at ?? 0));
      ...
      for (const event of events) {
        ...
        renderedContent[event.id] = await postProcessAsciidoctorHtml(html as string);
      }
  ...
  function getTags(event: NDKEvent): [string, string][] {
    ...
    return (event.tags || []).filter((tag): tag is [string, string] => tag.length >= 2);
  }
  ...
  $: filteredEvents = (() => {
    ...
    return events.filter(event => {
      ...
      const relevantTags = selectedTagTypes.size === 0
        ? tags
...
<div class="flex flex-row w-full max-w-none py-8 px-8 gap-24 min-w-0 overflow-hidden">
  <!-- Tag Filter Sidebar -->
      ...
      {#each tagTypes as type}
        <button
          class="px-3 py-1 rounded-full text-xs font-medium flex items-center gap-2 transition-colors
            ...
            {selectedTagTypes.has(type) ? 'border-2 border-amber-800' : 'border border-amber-200'}"
    ...
    <div class="flex flex-wrap gap-2 mb-4">
      {#each tagsToShow as tag}
        <button
          class="px-3 py-1 rounded-full text-xs font-medium flex items-center gap-2 transition-colors
            bg-amber-100 text-amber-900 hover:bg-amber-200
            {tagFilter.has(tag) ? 'border-2 border-amber-800' : 'border border-amber-200'}"
            ...
            {#if showTags[event.id]}
              <div class="mb-2 text-xs flex flex-wrap gap-2">
                {#each getTags(event) as tag}
                  <span class="bg-amber-900 text-amber-100 px-2 py-1 rounded-full text-xs font-medium flex items-baseline">
