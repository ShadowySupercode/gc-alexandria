...
export async function getUserMetadata(
  ...
  try {
    ...
    const decoded = client.encoding.decode(cleanId);
    if (!decoded) {
      npubCache.set(cleanId, fallback);
      return fallback;
    }
    ...
    const profile =
      profileEvent.event && profileEvent.event.content
        ? JSON.parse(profileEvent.event.content)
        : null;
...
export async function createProfileLinkWithVerification(
  ...
  let user: NostrUser | null;
  if (isNpub) {
    const decoded = client.encoding.decode(cleanId);
    if (decoded.type !== 'npub') {
      return createProfileLink(identifier, displayText);
    }
    user = await client.getUser(decoded.data);
  } else {
    user = await client.getUser(cleanId);
  }
  ...
  const userInboxRelays = client.getConnectedRelays();
  const allRelays = [
    ...communityRelays,
    ...userInboxRelays,
    ...fallbackRelays,
  ...
  const relaySet = client.getRelaySet(allRelays);
  const profileEvent = await client.fetchEvent(
...
/**
 * Get npub from NIP-05 identifier
 */
export async function getNpubFromNip05(nip05: string): Promise<string | null> {
  try {
    ...
    const user = await client.getUserFromNip05(nip05);
    if (!user) return null;

    return client.encoding.encodeNpub(user.pubkey);
  } catch (e) {
    console.error('Error getting npub from NIP-05:', e);
    return null;
  }
}
...
export function toNpub(pubkey: string | undefined): string | null {
  ...
  try {
    const client = getNostrClient();
    return client ? client.encoding.encodeNpub(pubkey) : null;
  } catch (e) {
