...
export async function fetchEventWithFallback(
  ...
  // Remove 'nostr:' prefix if present
  let cleanedFilterOrId: string | NDKFilter<NDKKind> = filterOrId;
  if (typeof filterOrId === "string") {
    cleanedFilterOrId = filterOrId.replace(/^nostr:/, "");
  }

  const primaryRelays =
    relays && relays.length > 0 ? relays : selectRelayGroup();
  ...
  async function searchRelays(
    relays: string[],
    group: string,
  ): Promise<RelaySearchResult | null> {
    ...
    const searchPromises = relays.map((relayUrl) => {
      ...
      return (async () => {
        ...
        try {
          if (
            typeof cleanedFilterOrId === "string" &&
            /^[0-9a-f]{64}$/i.test(cleanedFilterOrId)
          ) {
            event = await ndk
              .fetchEvent({ ids: [cleanedFilterOrId] }, undefined, relaySet)
              .withTimeout(timeoutMs);
          } else {
            const filterObj =
              typeof cleanedFilterOrId === "string"
                ? { ids: [cleanedFilterOrId] }
                : cleanedFilterOrId;
            const results = await ndk
              .fetchEvents(filterObj, undefined, relaySet)
              .withTimeout(timeoutMs);
            event =
    ...
    const successful = results.find(
      (r) => r.status === "fulfilled" && r.value && r.value.event,
    );
