  ...
  try {
    for (let i = 0; i < events.length; i++) {
      ...
      const fieldMatches = {
        ...
        titleFuzzy:
          levenshteinDistance(title, searchQuery) <=
          Math.min(3, searchQuery.length / 2),
        authorNameExact: authorName.includes(searchQuery),
        authorNameFuzzy:
          levenshteinDistance(authorName, searchQuery) <=
          Math.min(3, searchQuery.length / 2),
        summaryExact: summary.includes(searchQuery),
        summaryFuzzy:
          levenshteinDistance(summary, searchQuery) <=
          Math.min(3, searchQuery.length / 2),
    ...
    return results.sort((a, b) => b.score - a.score).map(({ event }) => event);
  } catch (error) {
...
function generateSuggestions(
  ...
  const suggestions = new Map<
    string,
    { text: string; type: "title" | "author" | "tag"; score: number }
  >();
...
self.onmessage = async (e: MessageEvent<SearchMessage>) => {
  ...
  if (type === "SEARCH") {
    try {
      const results = await processEventsInChunks(events, query, chunkSize);
