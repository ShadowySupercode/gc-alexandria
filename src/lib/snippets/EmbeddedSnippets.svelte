<script module lang="ts">
  ...
  function truncateRenderedContent(
    ...
    if (hasQuoteBoxes) {
      const quoteBoxPattern =
        /<div class="block w-fit my-2 px-3 py-2 bg-gray-200[^>]*onclick="window\.dispatchEvent\(new CustomEvent\('jump-to-message'[^>]*>[^<]*<\/div>/g;
  ...
  > {
    ...
    const profilePromises = Array.from(uniquePubkeys).map(async (pubkey) => {
      try {
        ...
        if (
          profile &&
          (profile.name || profile.displayName || profile.picture)
        ) {
        ...
        for (const relay of searchRelays) {
          try {
            ...
            console.warn(
              `[fetchAuthorProfiles] Failed to fetch profile from ${relay}:`,
              error,
            );
          }
        }
        ...
        try {
          ...
          if (allRelays.length > 0) {
            const ndkRelaySet = NDKRelaySetFromNDK.fromRelayUrls(
              allRelays,
              ndk,
            );
            const profileEvent = await ndk.fetchEvent(
          ...
          console.warn(
            `[fetchAuthorProfiles] Failed to fetch profile from all relays:`,
            error,
          );
        }
      } catch (error) {
        console.warn(
          `[fetchAuthorProfiles] Error processing profile for ${pubkey}:`,
          error,
        );
      }
    });
  ...
  async function findQuotedMessage(
    eventId: string,
    publicMessages: NDKEvent[],
    ndk: NDK,
    ...
    if (!quotedMessage) {
      try {
        ...
        console.warn(
          `[findQuotedMessage] Failed to fetch quoted event ${eventId}:`,
          error,
        );
      }
    }
...
{#snippet repostContent(content: string)}
  ...
  {#if originalEvent}
    ...
    {@const formattedDate = originalCreatedAt
      ? new Date(originalCreatedAt * 1000).toLocaleDateString()
      : "Unknown date"}
    {@const shortAuthor = originalAuthor
      ? `${originalAuthor.slice(0, 8)}...${originalAuthor.slice(-4)}`
      : "Unknown"}

    <div
      class="embedded-repost bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4 my-2"
    >
      <!-- Event header -->
      <div class="flex items-center justify-between mb-3 min-w-0">
        <div class="flex items-center space-x-2 min-w-0">
          <span
            class="text-xs text-gray-500 dark:text-gray-400 font-mono flex-shrink-0"
          >
            Kind {originalKind}
          </span>
            ...
            <div
              ...
              onclick={() =>
                window.dispatchEvent(
                  new CustomEvent("jump-to-message", { detail: eventId }),
                )}
              role="button"
              tabindex="0"
              onkeydown={(e) => {
          ...
          {#if isValidEventId}
            {@const nevent = (() => {
              try {
                ...
                console.warn(
                  `[quotedContent] Failed to encode nevent for ${eventId}:`,
                  error,
                );
                return null;
              }
            ...
            {#if nevent}
              ...
              >
                Quoted message not found. Click to view event {eventId.slice(
                  0,
                  8,
                )}...
              </div>
            {:else}
              <div
                class="block w-fit my-2 px-3 py-2 bg-gray-200 dark:bg-gray-700 border-l-2 border-gray-400 dark:border-gray-500 rounded text-sm text-gray-600 dark:text-gray-300"
              >
