<script lang="ts">
  ...
  let existingKinds = $derived(
    $visualizationConfig.eventConfigs.map((ec: any) => ec.kind),
  );
  ...
  function handleKeydown(e: KeyboardEvent) {
    handleEventKindKeydown(e, handleAddKind, () => {
      showAddInput = false;
      newKind = "";
      inputError = "";
    });
...
<div class="space-y-3">
  <span class="text-xs text-gray-600 dark:text-gray-400">
    Showing {Object.values(eventCounts).reduce((a: any, b: any) => a + b, 0)} of
    {Object.values(eventCounts).reduce((a: any, b: any) => a + b, 0)} events
  </span>
  ...
  <div class="space-y-2">
    {#each $visualizationConfig.eventConfigs as config}
      ...
      <div class="flex items-center gap-2">
        ...
        <button
          class="flex items-center gap-1 min-w-[140px] px-2 py-1 border-2 rounded {borderColor} {isDisabled
            ? 'opacity-50'
          ...
          title={isDisabled
            ? `Click to enable ${getEventKindName(config.kind)}`
            : `Click to disable ${getEventKindName(config.kind)}`}
        >
          <span
        ...
        {:else}
          ...
          <input
            ...
            min="1"
            max="1000"
            class="w-16 px-2 py-1 text-xs border rounded dark:bg-gray-700 dark:border-gray-600 dark:text-white {(config.kind ===
              30041 ||
              config.kind === 30818) &&
            config.showAll
              ? 'opacity-50'
            ...
            disabled={(config.kind === 30041 || config.kind === 30818) &&
              config.showAll}
          />
          ...
          {#if config.kind === 30041 || config.kind === 30818}
            ...
            >
              <input
                ...
                onchange={() =>
                  visualizationConfig.toggleShowAllContent(config.kind)}
                class="w-3 h-3"
              />
              All
            </label>
  ...
  {#if showAddInput}
    <div class="flex items-center gap-2">
      <input
        ...
        oninput={(e) => {
          const validation = validateEventKind(
