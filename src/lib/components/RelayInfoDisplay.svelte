<script lang="ts">
  ...
  import {
    ...
    getRelayTypeLabel,
    getRelayIcon,
    type RelayInfoWithMetadata,
  } from "$lib/utils/relay_info_service";
...
<div class="inline-flex items-center gap-2 flex-1">
  {#if showIcon && relayIcon}
    <img
      ...
      class="{iconSizeClasses[
        size as keyof typeof iconSizeClasses
  ...
  {:else if showIcon}
    ...
    <div
      class="{iconSizeClasses[
        size as keyof typeof iconSizeClasses
      ]} bg-gray-300 dark:bg-gray-600 rounded flex items-center justify-center"
    >
      <svg
        class="w-2/3 h-2/3 text-gray-600 dark:text-gray-400"
        fill="currentColor"
        viewBox="0 0 20 20"
      >
        <path
  ...
  <div class="flex flex-col min-w-0 flex-1">
    {#if showName}
      <span
        ...
        ]} font-medium text-gray-900 dark:text-gray-100 leading-tight truncate"
      >
    ...
    {#if showType}
      <span
        class="text-xs text-gray-500 dark:text-gray-400 leading-tight truncate"
      >
        {relayType}
      </span>
    {/if}
  ...
  {#if error}
    <span
      class="text-xs text-red-500 dark:text-red-400 flex-shrink-0"
      title={error}
    >
      ⚠️
    </span>
