    ...
    if (Number(eventData.kind) === 30040) {
      ...
      try {
        ...
        const compatibleTags: [string, string][] = tags
          .filter(
            (tag) => tag.key.trim() !== "" && !presetTagKeys.includes(tag.key),
          )
        ...
        const finalTags = indexEvent.tags.filter(
          (tag) => !presetTagKeys.includes(tag[0]),
        );
...
<div
  class="mt-6 border-t border-gray-200 dark:border-gray-700 pt-4 overflow-hidden"
>
  ...
  {#if showJsonPreview}
    {#if eventPreview}
      <div
        class="bg-gray-50 dark:bg-gray-800 rounded-lg p-4 border border-gray-200 dark:border-gray-600 overflow-hidden"
      >
        ...
        {:else}
          <div class="mb-2">
            <span class="text-sm font-medium text-gray-700 dark:text-gray-300">
              Event Type: {eventPreview.type === "30040_index_event"
          ...
          <div class="overflow-hidden">
            <pre
              class="text-xs bg-white dark:bg-gray-900 p-3 rounded border overflow-x-auto text-gray-800 dark:text-gray-200 font-mono whitespace-pre-wrap break-words max-w-full">{JSON.stringify(
                eventPreview.event,
                null,
                2,
              )}</pre>
