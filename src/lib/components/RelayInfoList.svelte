<script lang="ts">
  ...
  const groupedRelays = $derived(() => {
    ...
    return {
      ...
      inbox: categorized.filter(
        (r: CategorizedRelay) => r.category === "inbox",
      ),
      outbox: categorized.filter(
        (r: CategorizedRelay) => r.category === "outbox",
      ),
      other: categorized.filter(
        (r: CategorizedRelay) => r.category === "other",
      ),
    };
  });
  ...
  const categoryColors = {
    ...
    outbox:
      "bg-purple-100 dark:bg-purple-900 border-purple-200 dark:border-purple-700 text-purple-800 dark:text-purple-200",
    other:
