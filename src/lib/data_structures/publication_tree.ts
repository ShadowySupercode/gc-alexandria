  ...
  async #depthFirstRetrieve(address?: string): Promise<NDKEvent | null> {
    ...
    while (stack.length > 0) {
      ...
      if (currentChildAddresses.length === 0) {
        ...
        const eTagPromises = eTags.map(async (tag) => {
          try {
            ...
            if (referencedEvent) {
              ...
              const dTag = referencedEvent.tags.find(
                (tag) => tag[0] === "d",
              )?.[1];
              if (dTag) {
                const address =
        ...
        const validAddresses = resolvedAddresses.filter(
          (addr) => addr !== null,
        ) as string[];
  ...
  async #resolveNode(
    ...
    if (!event) {
      ...
      return fetchEventWithFallback(
        ...
        5000,
      ) // 5 second timeout for publication events
        .then((fetchedEvent) => {
          if (fetchedEvent) {
    ...
    try {
      ...
      for (const relay of uniqueRelays) {
        try {
          ...
          const fetchedEvent = await this.#ndk
            .fetchEvent(
              {
                kinds: [parseInt(kind)],
                authors: [pubkey],
